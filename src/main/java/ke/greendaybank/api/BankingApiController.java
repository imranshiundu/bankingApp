...
public class BankingApiController {
    ...
    public BankingApiController(BankingApplicationService service, ApprovalExecutionService approvalExecution, IdempotentOperationRunner idempotentRunner, ApprovalPolicy approvalPolicy) {
    ...
    public ResponseEntity<String> move(@RequestBody @Valid MovementRequest request) {
        if (approvalPolicy.movementRequiresApproval(request.amount())) {
            UUID approvalId = approvalExecution.requestHighRiskMovement(request.fromAccountNumber(), request.toAccountNumber(), request.amount(), request.narration(), request.idempotencyKey(), actor(request.actor()));
            return ResponseEntity.status(HttpStatus.ACCEPTED).body("{\"status\":\"APPROVAL_REQUIRED\",\"approvalId\":\"" + approvalId + "\",\"thresholdKes\":\"" + approvalPolicy.highRiskKesThreshold() + "\"}");
        }
