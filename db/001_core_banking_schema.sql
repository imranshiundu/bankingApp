...
CREATE TABLE security.staff_users (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE TABLE security.staff_user_roles (
...
-- =========================================================
-- CUSTOMER IDENTITY AND ENCRYPTED PROFILE VAULT
-- =========================================================

CREATE TABLE core.customers (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    customer_ref TEXT NOT NULL UNIQUE,
    status core.account_status NOT NULL DEFAULT 'PENDING_KYC',
    created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
    updated_at TIMESTAMPTZ NOT NULL DEFAULT now()
);

CREATE TABLE core.customer_encrypted_profiles (
    customer_id UUID PRIMARY KEY REFERENCES core.customers(id) ON DELETE CASCADE,
    vault_version TEXT NOT NULL,
    encryption_algorithm TEXT NOT NULL CHECK (encryption_algorithm = 'Twofish/CBC/PKCS7Padding'),
...
CREATE TABLE core.customer_kyc_documents (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    ...
    verified_by UUID REFERENCES security.staff_users(id),
...
CREATE TABLE core.transactions (
    ...
    requested_by UUID REFERENCES security.staff_users(id),
    posted_at TIMESTAMPTZ,
    created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
    reversed_transaction_id UUID REFERENCES core.transactions(id)
);
...
CREATE TABLE core.ledger_entries (
    ...
    side core.ledger_side NOT NULL,
...
CREATE VIEW core.account_balances AS
SELECT
    a.id AS account_id,
    a.account_number,
    a.currency,
    COALESCE(SUM(CASE WHEN le.side = 'CREDIT' THEN le.amount ELSE -le.amount END), 0.00) AS balance
FROM core.accounts a
LEFT JOIN core.ledger_entries le ON le.account_id = a.id
GROUP BY a.id, a.account_number, a.currency;

-- =========================================================
-- POSTING CONTROL: TRANSACTION MUST BALANCE
-- =========================================================

CREATE OR REPLACE FUNCTION core.assert_transaction_balanced(p_transaction_id UUID)
RETURNS VOID AS $$
DECLARE
    imbalance_count INT;
BEGIN
    ...
    IF imbalance_count > 0 THEN
        RAISE EXCEPTION 'Unbalanced transaction %', p_transaction_id;
    END IF;
END;
$$ LANGUAGE plpgsql;

-- Application layer must call assert_transaction_balanced before marking status POSTED.
-- Database triggers can be added after batch-posting procedure is finalized.
...
CREATE TABLE approvals.approval_requests (
    ...
    status approvals.approval_status NOT NULL DEFAULT 'PENDING',
    requested_by UUID NOT NULL REFERENCES security.staff_users(id),
    reviewed_by UUID REFERENCES security.staff_users(id),
...
CREATE TABLE audit.events (
    ...
    actor_user_id UUID REFERENCES security.staff_users(id),
...
CREATE TABLE compliance.aml_screening_cases (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    customer_id UUID NOT NULL REFERENCES core.customers(id),
...
CREATE TABLE compliance.suspicious_activity_reports (
    ...
    filed_by UUID REFERENCES security.staff_users(id),
